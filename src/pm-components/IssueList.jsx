...
export default function IssueList({ repos, stateFilter, repoFilter, onSelectItem, refreshSignal, freshItem }) {
  ...
  if (error && issues.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 40, gap: 12 }}>
        ...
        <button
          onClick={() => fetchIssues()}
          style={{ background: "var(--control-bg)", border: "1px solid var(--control-border)", borderRadius: 6, color: "var(--text-tertiary)", padding: "6px 16px", cursor: "pointer", fontFamily: "system-ui", fontSize: 12 }}
        >
          Retry
        </button>
      </div>
  ...
  if (issues.length === 0) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 40, color: "var(--text-faint)", fontFamily: "system-ui", fontSize: 13 }}>
        No issues found
      </div>
    );
  }
        ...
        return (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {isOpen ? (
                <Circle size={12} color="var(--badge-open-text)" />
              ) : (
                <CheckCircle2 size={12} color="var(--badge-merged-text)" />
              )}
              <span style={{ color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                #{item.number}
              </span>
              <span style={{ color: "var(--text-secondary)", fontFamily: "system-ui", fontSize: 13, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {item.title}
              </span>
              <HoverIconButton
                className="copy-btn"
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                <span style={{ width: 25, display: "inline-flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)" }} title={linkedPRs[`${item._repo}/${item.number}`] ? `${linkedPRs[`${item._repo}/${item.number}`].length} linked PR${linkedPRs[`${item._repo}/${item.number}`].length > 1 ? "s" : ""}` : undefined}>
            ...
            <div style={{ marginLeft: 26, color: "var(--text-faint)", fontFamily: "system-ui", fontSize: 12, marginTop: 2 }}>
