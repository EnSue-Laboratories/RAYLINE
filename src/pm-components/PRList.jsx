...
export default function PRList({ repos, stateFilter, repoFilter, onSelectItem, refreshSignal, freshItem, locale = "en-US" }) {
  ...
  function getIcon(item) {
    if (item.state === "open") {
      return <GitPullRequest size={12} color="var(--badge-open-text)" />;
    }
    if (item.merged_at) {
      return <GitMerge size={12} color="var(--badge-merged-text)" />;
    }
    return <GitPullRequestClosed size={12} color="var(--badge-closed-text)" />;
  }
