...
export default function useAgent() {
  ...
  const startPreparedMessage = useCallback(({ conversationId, pendingId, sessionId, prompt, model, provider, effort, thinking, openCodeConfig, providerUpstreamConfig, networkProxy, cwd, projectContext, images, files, resumeSessionId, forkSession, multicaContext, multicaToken }) => {
    ...
    if (window.api) {
      const payload = { conversationId, sessionId, prompt, model, provider, effort, thinking, openCodeConfig, providerUpstreamConfig, networkProxy, cwd, projectContext, images, files, resumeSessionId, forkSession };
      if (provider === "multica") {
  ...
  const editAndResend = useCallback(({ conversationId, sessionId, messageIndex, newText, wirePrompt, model, provider, effort, thinking, openCodeConfig, providerUpstreamConfig, networkProxy, cwd, projectContext, multicaContext, multicaToken }) => {
    setConversations((prev) => {
      const next = new Map(prev);
      const convo = next.get(conversationId);
      if (convo) {
